"""
Voice IT - ChatGPT Provider
Uses OpenAI's API for speech-to-text transcription (Whisper) and text transformation (GPT-4).
"""
...
class ChatGPTAdapter(BaseProvider):
    """
    ChatGPT provider using OpenAI's API for transcription and text transformation.

    Uses Whisper API for audio transcription and GPT-4 for text operations.
    """
    ...
    async def authenticate(self, api_key: str = None) -> bool:
        ...
        Args:
            api_key: OpenAI API key (if not provided, uses stored key)
        ...
        try:
            ...
            if not client:
                return TranscriptionResult(
                    text="",
                    error="Not authenticated. Please connect ChatGPT first.",
                )
